...
static wm_singleton: Option<WindowMiddleware> = None;
impl Singleton<WindowMiddleware> for WindowMiddleware
{
    fn init<'a>() -> &'a mut Self
    {
        if wm_singleton.is_some()
        {
            panic!("Cannot initialize twice");
        }
        wm_singleton = Some(Self::new());
        wm_singleton
    }
    fn uninit()
    {
    ...
    fn get<'a>() -> Option<&'a mut Self> { wm_singleton }
}
