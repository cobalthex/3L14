...
impl DebugDraw
{
    pub fn new(renderer: &Renderer) -> Self
    {
        ...
        let lines_vbuffer = renderer.device().create_buffer(&BufferDescriptor
        {
            ...
            usage: BufferUsages::COPY_DST | BufferUsages::,
    ...
    {
        let start = self.lines.vertices.len() as u32;
        let points = f32::lerp(8.0, 16.0, radius) as u32;

        // TODO: this is wrong
        let aspect = self.camera_transform.y_axis.y / self.camera_transform.x_axis.x;
        ...
        for i in 0..points
        {
            self.lines.vertices.push(DebugLineVertex
            {
                ...
                [
                    ...
                    f32::sin(i as f32 * inc) * radius * aspect + center.y,
                    0.0,
                    1.0
                ],
                color: color.into(),
