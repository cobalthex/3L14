...
impl std::fmt::Display for MapBuildError
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result
    {
        match self
        {
            ...
            MapBuildError::DependencyError(e) => write!(f, "Error loading dependency: {}", e),
        }
    }
}
...
impl AssetBuilder for MapBuilder
{
    ...
    fn build_assets(&self, config: Self::BuildConfig, input: &mut SourceInput, outputs: &mut BuildOutputs) -> Result<(), Box<dyn Error>>
    {
        ...
        let mut map = outputs.add_output::<Map>()?;
        ...
        let mut insert_model = |model_key: asset_3l14::AssetKey|
        {
            ...
            match entry
            {
                Entry::Vacant(vac) =>
                {
                    let model = map.add_and_read_dependency::<Model>(model_key).map_err(MapBuildError::DependencyError)?;
                    Ok(*vac.insert((new_index as u32, model.bounds_aabb)))
                }
                Entry::Occupied(occ) =>
                {
                    Ok(*occ.get())
